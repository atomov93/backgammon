	...
	async messageHandler(data) {
		...
		switch (action) {
			...
			case "1": // CONNECT_TO_ROOM
				if (!message || !message.roomName || !message.username) {
					console.error("Invalid message format for CONNECT_TO_ROOM");
					return;
				}
				const { roomName, username } = message;
				console.log(
					`CONNECT_TO_ROOM action triggered for room: ${roomName}, username: ${username}`
				);
	...
	async connectToRoom(roomName) {
		...
		try {
			...
			if (!gameRoom) {
				...
				console.log(
					`Found existing room: ${roomName} with players: ${gameRoom.players.join(
						", "
					)}`
				);
			}
			...
			if (gameRoom.players.includes(this.username)) {
				console.log(
					`User ${this.username} is already in the room: ${roomName}`
				);
				return;
			}
